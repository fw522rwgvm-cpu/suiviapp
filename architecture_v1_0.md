...
| 5 | **DIRECTION iOS 26, §5** | **`GlassContainer` entre dans le projet**, et c'est lui qui manquait | Demande explicite (specs §14.26 n° 4). Aucune dependance n'entre : `expo-glass-effect` est au §5 depuis la tranche 3 et exposait deja cette vue, jamais utilisee. **Verifie dans la source du paquet plutot que suppose** : `GlassContainer.swift` construit un `UIGlassContainerEffect`, lui passe son `spacing`, et teste lui-meme `NSClassFromString("UIGlassContainerEffect")` — donc il degrade sans planter la ou la classe n'existe pas. La ligne de partage reste « la surface porte-t-elle deja un materiau », et elle descend d'un cran : la bande n'en porte pas, les controles si |
