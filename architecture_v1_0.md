...
| 5 | **DIRECTION iOS 26** | **La barre d'accessoire passe au verre, et la regle de l'en-tete ne s'y applique pas** | Demande explicite (specs §14.26 n° 4). « Jamais de `GlassButton` dans un en-tete natif » tient parce qu'un en-tete pose **deja** le materiau d'UIKit derriere ce qu'on lui donne. Un `InputAccessoryView` est un conteneur **vide** : rien ne lui est applique, donc `GlassView` y est la seule facon d'obtenir le materiau, ce que la direction iOS 26 dit deja de tout contenu dessine en JavaScript. La ligne de partage n'est pas « natif contre maison », c'est **« la surface porte-t-elle deja un materiau »** |
| 6 | **§4** | **`GlassButton` gagne `disabled`** | Les chevrons des extremites d'un formulaire. Ils restent DESSINES plutot que retires : une barre dont le contenu apparait et disparait au fil du focus est une barre qui saute, et c'est la forme de la rangee qui dit ou ils sont avant qu'on les lise. La teinte passe a `textFaint`, comme les fleches qu'ils remplacent |
