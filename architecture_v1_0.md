...
| 8 | **Médias d'exercices hors filet JSON** | **[tranche 11] Scindé.** Vrai d'un média CHOISI par l'utilisateur, qui reste couvert par la seule copie manuelle. Faux d'un dessin du catalogue : `media_uri` y porte une clé préfixée `catalog:`, le dessin est dans le binaire, et la valeur revient donc **vivante** après un aller-retour (§9.24 n° 17) |
...
**[livrée]** Avec dix-sept amendements, dont quatre qui comptent : les tables arrivent par **`0010`** et non `0009` — dépensée sur les durées — ; l'invariant « une seule séance en cours » est une **paire** index + CHECK, un index partiel ne contraignant rien hors de son prédicat ; la fin d'un segment est estampillée par l'**écriture suivante**, seule forme qui survive à un arrêt forcé ; et le minuteur de repos n'est **pas** une `NOTIFICATION_KIND`, contrairement à ce que la tranche 9 avait inscrit deux fois. S'y ajoute une base d'exercices livrée, offerte et jamais installée. Voir §9.24.
...
**[livrée]** **Sans migration**, et la règle le justifie : tout ce que la tranche affiche est dérivé, et les deux index dont elle vit existent depuis `0008` et `0010`. Sa forme tient en une phrase — **une lecture, trois plis** : les cinq graphiques du §10.1, les quatre records et la condition du §10.4 sont trois questions sur les mêmes lignes. Trois décisions comptent au-delà : `touchSession` refuse de prolonger une séance terminée, sans quoi corriger une charge trois semaines plus tard gonflait son temps actif ; la page d'un exercice lit son historique par identifiant seul, parce que le §5.3 promet que la suppression rompt la continuité ; et le générateur écrit enfin des séances, ce que D15 lui demandait depuis la tranche 10. Voir §9.35.
...
| 4 | **§4, D16** | **`onContentSizeChange` est l'evenement « ce champ vient de grandir »** | Demande explicite (specs §14.28 n° 3). Pas de sondage et pas de mesure a chaque frappe : le champ le dit lui-meme, et le revelage qu'il demande rend **zero** quand le champ a encore de la place — c'est cette idempotence qui le rend sur a appeler a chaque ligne gagnee, comme elle rend sur d'appeler au focus ET a l'arrivee du clavier |
| 5 | **§4** | **Un champ plus haut que la bande visible garde son BAS** | Sinon les deux corrections de `shiftToReveal` s'appliquent en meme temps et se repoussent a chaque ligne tapee. Le bas gagne parce que c'est la qu'est le curseur : ce qu'on ecrit doit se voir, le debut d'une note en cours d'ecriture non. **Deux tests fixent la bagarre**, dont celui qui rougit sans le garde — un va-et-vient est le seul defaut de cette famille qu'on ne verrait pas en lisant le code |

---

### 9.24 Tranche 11 (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§7, §9.15 n° 1** | **Les cinq tables de seance arrivent par `0010`**, pas `0009` | Les deux documents annoncaient `0009` ; elle a ete depensee sur les durees apres les retours de la tranche 10 (§9.16 n° 1). Les deux sont corriges plutot que laisses en contradiction avec le journal des migrations, qui est la seule source qui compte |
| 2 | **§2.6, D12** | **L'invariant « une seule seance en cours » est une PAIRE** : `ux_session_active` ET `ck_session_status` | Un index partiel `WHERE status = 'in_progress'` ne contraint RIEN sur une ligne dont le statut dit autre chose, donc une archive posant deux seances a `'running'` importerait proprement et l'application en tiendrait deux vivantes. C'est exactement la contournabilite que D12 exige d'impossible, atteinte par la route que D12 dit d'eviter. La CHECK est ce qui rend l'index total, et c'est le seul endroit du schema ou c'est vrai d'un index. Second motif : un index unique nomme la COLONNE dans son erreur, pas l'index — constat de la tranche 8 — la ou une CHECK dit son propre nom |
| 3 | **§2.6** | **L'objection de la tranche 5 contre un index unique partiel ne s'applique pas ici** | `day_meal` avait vu le sien refuse parce qu'une base en service portait deja les lignes qui le violeraient : il aurait echoue a se construire sur exactement les donnees qu'il existait pour proteger. `session` est NEUVE — aucune ligne dans aucune base ni aucune archive — donc il se construit toujours. C'est l'argument de `weight_goal.is_active` en tranche 8, mot pour mot |
| 4 | **§2.6** | **`session_set` gagne `target_duration_seconds` et `actual_duration_seconds`** | Le §2.6 precede `0009` et ne decrit une serie que par ses repetitions. Sans elles, un exercice marque `tracks_duration` se met dans une routine et ne s'execute jamais |
| 5 | **§2.6, D5/R4** | **`session_set.exercise_id` est NO ACTION, et `deleteExercise` porte la politique** | La cle etrangere aurait fait ECHOUER la suppression des la premiere seance enregistree — et le §5.3 dit qu'aucune suppression n'est bloquee. Rien ne l'aurait signale avant qu'une vraie seance existe. Le schema porte deja la reponse en deux colonnes : l'id est nullable, `exercise_name_frozen` est NOT NULL. La transaction delie ; le nom survit. Precedent exact : `recipe_ingredient.food_id` et `deleteFood` |
| 6 | **D12** | **La fin d'un segment est estampillee par l'ecriture SUIVANTE, jamais par un minuteur** | D12 dit « se ferme apres 30 minutes sans aucune ecriture », ce qui est inimplementable lu comme une consigne a un minuteur : rien de l'application ne tourne en arriere-plan ou apres un kill (§2.2), donc la fermeture n'aurait aucun moment ou se produire. Lu comme une regle sur la prochaine ecriture, il n'y a rien a faire tourner entre les deux. **Corollaire : `ended_at` n'est jamais NULL pendant que l'application tourne** — la forme evidente oblige a deviner une fin a la lecture, et `COALESCE(ended_at, now)` compte la nuit entiere |
| 7 | **D12, D9** | **Le minuteur de repos ne stocke rien** : `session_set.completed_at` est l'instant de depart que D12 demande | Il existait deja, ecrit dans la meme transaction que la serie. Le decompte est entierement derive, donc aucune colonne, aucune ligne, aucun etat — et il survit a un arret force parce que la seance y survit |
| 8 | **D14, §2.5** | **Le minuteur n'est PAS une `NOTIFICATION_KIND`**, et les deux commentaires qui disaient le contraire sont corriges | `diffSchedule` decide ce qui appartient au planificateur en testant si un identifiant commence par une sorte, et annule tout ce qu'il possede et qui n'est pas dans le plan — a chaque passage au premier plan. Ajouter `rest_timer` aurait confie le minuteur a un planificateur qui n'en a jamais entendu parler, silencieusement. La tranche 9 avait ecrit le paragraphe qui l'explique ET l'invitation qui le contredit, dans `notifications.ts` et dans le catalogue d'export. La propriete est INVISIBLE — elle tient a ce que deux constantes ne se rencontrent pas — donc un test la nomme |
| 9 | **D14** | **`NotificationHost` gagne `scheduleAfter`**, et le meme constat de la tranche 9 donne les deux reponses | Le declencheur `DATE` d'expo est un delai fige a la planification : faux pour une date, qui derive sur sept jours et un changement d'heure, et exactement juste pour un compte a rebours de quatre-vingt-dix secondes. Une methode separee plutot qu'un drapeau, pour qu'aucun des deux appelants ne puisse atteindre l'autre |
| 10 | **§3, D9** | **`restForBlock` prend les deux champs qu'elle lit**, plus un `BlockDraft` | La tranche lui donne un second appelant dont les blocs viennent de SQL, et les deux formes different par un id que cette resolution ne regarde jamais. Elargie structurellement plutot que dupliquee : quel repos s'applique est UNE regle, et une seconde ecriture est la facon dont une seance et la routine dont elle vient finiraient par prescrire des repos differents |
| 11 | **§3** | **`Cell` sort de `set-table.tsx` dans `set-cell.tsx`**, a son deuxieme utilisateur reel | La regle du second utilisateur, appliquee. **Dans la feature, pas dans `core/ui`** : elle sait qu'elle est une cellule de table de series, avec son alignement et son tiret pour « rien d'indique ». `core/ui/decimal-input` est la forme generique et enveloppe une rangee de FORMULAIRE, qui est l'autre idiome. Deux formes, deux composants, une regle partagee en prose — dit ici parce que les fusionner est le rangement tentant |
| 12 | **DEFAUT LIVRE** | **`exercise.tracks_duration` etait relue partout et ecrite nulle part depuis `0009`** | Absente d'`ExerciseDraft`, de `columnsOf` et de `readExerciseDraft` : `0` sur tous les exercices ayant jamais existe, colonne « Temps » inatteignable, `routine_line.duration_seconds` hors de portee. Meme defaut que le §9.18 n° 2 un etage plus bas, trouve de la meme facon — en touchant une projection — et pour la meme raison : la lecture rendait fidelement le defaut que l'ecriture n'avait jamais ecrase, donc l'aller-retour etait coherent et faux |
| 13 | **DEFAUT DANS UN TEST** | **`sameExerciseDraft`'s test est desormais indexe par `keyof ExerciseDraft`** | Il existe precisement pour un champ ajoute et oublie (§9.22 n° 2) et sa liste etait un TABLEAU : `tracksDuration` a ete ajoute au brouillon et le test est reste vert — le jour ou il devait rougir. Un `Record` fait poser la question par le compilateur, qui nomme le champ manquant. Verifie par mutation. **La regle qui en sort : un test qui garde l'exhaustivite d'un type doit etre indexe PAR ce type, jamais par des exemples** |
| 14 | **§3, D10** | **Le bandeau de seance monte au-dessus de tout le routeur**, et sa position porte une reserve | Le §10.3 demande « dans toute l'application », donc pas un element d'ecran. La barre d'onglets est un vrai `UITabBarController` et `useBottomTabBarHeight` appartient au navigateur JavaScript — il leve ici — donc sa hauteur n'est pas lisible. 49 points declares, hauteur d'une `UITabBar` en portrait depuis iOS 7 : une constante de PLATEFORME, pas une supposition sur notre propre contenu, ce qui est la distinction que la regle de la tranche 7 trace. **Le sens de l'erreur possible est ce qui rend ca acceptable** : sur iOS 26 la barre se minimise, donc un decalage fixe laisse un jour et non un recouvrement |
| 15 | **§3** | **`app/(tabs)/training/session.tsx` et `catalog.tsx`**, dans la pile de l'onglet | Cinquieme application du motif de `(journal)` : poussees de la racine elles recouvriraient la barre d'onglets. Une seance est un endroit ou l'on RESTE et depuis lequel on doit pouvoir consulter le Journal — le contraire exact de la bibliotheque, qui a remonte a la racine en §9.19 n° 3 |
| 16 | **§5, D13** | **Aucune dependance n'entre, donc aucun cycle CI** | Les dessins sont des chaines de chemins dans un module TypeScript, le mecanisme de `paths.generated.ts` : du JavaScript. `react-native-svg` est dans le binaire depuis la tranche 7 et D13 nommait deja cet usage. **Cout mesure : 6,3 Mo au depart, 6,4 avec tout le code de la seance, 7,6 avec les dessins** — les 1,2 Mo annonces, et le compte d'exercices est une molette |
| 17 | **§6 n° 8** | **Les medias d'exercices ne sont plus entierement hors du filet** | Le point ouvert disait « couverts uniquement par la copie manuelle du dossier ». Vrai d'un media choisi par l'utilisateur ; faux d'un dessin du catalogue, qui est du code et revient VIVANT apres un aller-retour parce qu'il est dans le binaire. Le point est scinde plutot que clos |

---

### 9.25 Retours sur le catalogue (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§5, §9.24 n° 16** | **La source du catalogue passe a wger**, et le chiffre de bundle du 9.24 n° 16 est faux | Demande explicite. wger CONTIENT everkinetic — une large part de ses images leur est creditee, deja traduites — donc le changement ne troque rien : c'est le meme dessin plus cinq cents exercices, le nommage fait. 520 exercices, 194 dessins |
| 2 | **D16** | **Un asset est structurellement moins cher qu'un chemin inline**, et c'est la mesure qui decide | Un SVG inline est une constante de chaine que Hermes analyse a CHAQUE demarrage a froid : 40 Ko par exercice, et les chemins ne pouvaient pas etre raccourcis — ils contiennent des **arcs**, le cas que le §9.15 n° 6 a documente comme indecidable a re-serialiser. Un PNG n'est jamais analyse. **Mesure : 6,3 Mo avant la tranche, 7,6 avec 32 dessins SVG, 6,5 avec 520 exercices et 194 dessins PNG** — le bundle JavaScript redescend et le cout part dans 11 Mo d'assets charges a l'affichage |
| 3 | **§4** | **Une image du catalogue ne prend PAS le theme**, et c'est le seul endroit | L'encre de ces fichiers est noire ; sur une carte sombre le dessin serait un trou. La carte est donc blanche dans les deux themes, ce qui se lit comme une photographie. Inverser en sombre demanderait de traiter les pixels a la compilation pour un resultat qui ressemble a un negatif |
| 4 | **§3** | **`catalog.generated.ts` et `images.generated.ts` sont deux modules**, sur le precedent de `core/theme/nunito-assets` | `require` d'un `.png` parle a Metro et pas a Node. La coupure est ce qui laisse le catalogue — la partie qu'un test lit — etre importe depuis Node. Meme raison exactement que pour la table des fontes |
| 5 | **§4, D9** | **Une regle de nom PRIME sur wger pour trois muscles, et sur aucun autre** | wger ne modelise ni lombaires, ni avant-bras, ni adducteurs. La premiere version ne laissait une regle de nom que COMPLETER un primaire absent, ce qui n'en a atteint aucun : wger dit toujours quelque chose, donc `primaries[0]` gagnait toujours et une hyperextension revenait en ischio-jambiers. Or cette reponse n'est pas differente de la notre, elle est **la moins fausse que wger puisse donner** — deferer a un vocabulaire pour un muscle qu'il ne sait pas exprimer, c'est deferer a un choix force |
| 6 | **DEFAUT DANS UN TEST** | **Six tests nommaient des cles du catalogue en litteraux** | C'etait juste tant que trente-trois entrees etaient ecrites a la main dans le fichier d'a cote. Le catalogue etant genere, un litteral est un pari sur le nommage d'un tiers — et au changement de source six tests ont rougi sur des cles disparues plutot que sur un comportement qui aurait bouge. Ils indexent le catalogue desormais, ce qui les garde sur le chemin d'ECRITURE, qui est leur sujet |
| 7 | **D15** | **La licence ne depend pas du nombre d'utilisateurs**, ecrit une fois pour toutes | D15 fait du depot un depot PUBLIC et les IPA sont publiees en release : deux diffusions. Le droit d'auteur porte sur la copie et la diffusion, pas sur l'usage prive, donc « seul moi l'utilise » ne dit rien de l'acte qui compte. CC-BY-SA ne coute rien qui vaille un risque, et le seul contournement propre — depot prive et releases privees — couterait les minutes de CI gratuites que D15 nomme comme la contrainte la plus penible du projet |

### 9.26 Second retour sur le catalogue (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§3** | **`src/dev/reset.ts`**, a cote de `src/dev/seed.ts` | Le §3 ne detaille pas `dev/`. Le decoupage suit celui que la tranche 0 avait deja fait : ce qui n'existe que dans la variante de developpement vit ensemble, et le garde-fou `APP_VARIANT` est au composant, pas a la fonction — une fonction pure qui vide une base est testable, un composant qui se refuse a se dessiner ne l'est pas |
| 2 | **D15** | **Le reset SUPPRIME DES LIGNES, il ne supprime pas le fichier**, et c'est force | Supprimer `Documents/SQLite/suivi.db` serait le reset evident et il est impossible de l'interieur : la connexion est un singleton ouvert une fois, et rien du §5 ne sait redemarrer une application — c'est exactement ce que la tranche 2 avait etabli en devant **basculer** une base en place plutot qu'en rouvrir une. Retirer le fichier sous une poignee ouverte laisse un WAL que personne ne relit. Les lignes partent, le schema reste, et le resultat **est** l'etat qu'une installation neuve atteint apres ses migrations |
| 3 | **D6, D8** | **La liste des tables a vider est DERIVEE**, `allSchemaTableNames()` | Une liste tenue a la main serait le defaut meme que le catalogue d'export existe pour empecher, dans le seul endroit ou se tromper est invisible : un reset qui oublie une table a l'air d'avoir marche, et les lignes restantes ressortent en donnees impossibles trois jours plus tard. Une table ajoutee en tranche 12 est videe sans que personne s'en souvienne. `PRAGMA foreign_keys` **hors** de la transaction, la tranche 2 ayant deja paye pour apprendre qu'il y est silencieusement ignore |
| 4 | **D6, G4** | **L'installation par defaut n'est PAS un remplissage de migration**, et la raison n'a pas bouge | G4 rejoue les migrations sur une base peuplee a chaque import : un remplissage dans `0010` reinjecterait ces lignes, avec des ULID neufs, dans une archive qui n'en portait deliberement aucune. Ce qui a change est **qui demande**, pas ou c'est ecrit — un hook au lancement au lieu d'un bouton |
| 5 | **D16** | **Apres la premiere peinture, jamais dans la sequence de demarrage** | Les cinq etapes de la sequence doivent toutes finir avant qu'on dessine quoi que ce soit. Y glisser deux cents insertions mettrait une ecriture que personne n'attend devant les 1,5 s que D16 budgete pour qu'un chiffre soit lisible a froid. Ici la Journee est deja a l'ecran, et le bus de D8 fait apparaitre la bibliotheque quand elle atterrit — celui qui ouvre l'onglet dans l'instant la voit se remplir, ce qui est honnete : elle **est** en train de se remplir |
| 6 | **§2.2** | **Le drapeau est une ligne `setting`, donc il traverse l'export** | `catalog_seeded_at` n'a besoin d'aucune colonne, d'aucune table et d'aucune migration. Et le catalogue d'export ne pose **aucune** regle `one_of` sur `setting.key` — verifie, pas suppose — donc une cle nouvelle passe l'import sans code en plus. C'est ce qui fait qu'une bibliotheque videe exprès le reste apres un aller-retour, et qu'une archive anterieure a cette tranche recoit les defauts au prochain lancement, exactement comme une installation neuve |
| 7 | **D15** | **free-exercise-db refuse : aucun fichier LICENSE (404), et des photographies d'une personne identifiable** | Un badge « Unlicense » dans un README n'est pas une licence accordee, et le droit a l'image d'un modele de studio ne se regle pas par une licence logicielle meme quand elle existe. Le §9.25 n° 7 avait deja tranche la question de l'usage prive : le depot est public et les IPA sont en release, ce sont deux diffusions |

### 9.27 Passage a free-exercise-db (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§5, §9.25 n° 1** | **Troisieme source du catalogue**, et les chiffres du 9.25 sont peries | everkinetic (293), puis wger (520), puis free-exercise-db (876). **Mesures : 1746 photographies, 30,8 Mo d'assets, bundle JavaScript de 6,5 a 7,0 Mo.** Le bundle ne prend que 0,5 Mo pour 356 exercices de plus parce qu'une image reste un ASSET — Hermes ne la lit jamais — exactement la propriete que le 9.25 n° 2 avait mesuree contre les SVG inlines |
| 2 | **D15** | **Les images sont redimensionnees a la compilation du catalogue**, 400 px, qualite 65 | A la source elles font 103 Mo. Redimensionnees, 30,8 Mo — mesure sur les 1746, pas extrapolee, et le resultat a ete **regarde** avant d'etre fige. 400 px est la bonne taille pour une vignette de 44 pt et pour deux images cote a cote sur une fiche |
| 3 | **§5** | **`jimp-compact` est utilise et n'est PAS declare**, et c'est borne a ce cas | Aucun outil d'image n'existe sur cette machine — ni PIL, ni ImageMagick, ni sharp : c'est ce qui avait force `generate-icons.mjs` a ecrire un PNG a la main. `jimp-compact` est du JavaScript pur, present en dependance **transitive** de la chaine Expo. Acceptable ici et nulle part ailleurs : le script tourne a la main, sa sortie est commitee, et **rien de ce qu'il produit n'est importe par l'application**. S'il disparait, le script echoue bruyamment sur son `require` et les assets deja commites continuent de marcher |
| 4 | **§2.6, §4** | **`EXERCISE_IMAGES` passe de `Record<string, number>` a `Record<string, readonly number[]>`** | Deux images par exercice, et **l'ordre est le mouvement** : [0] depart, [1] arrivee. `ExerciseDrawing` prend un `poses` qui vaut `first` par defaut — deux vignettes de 44 pt ne sont deux fois rien — et `both` sur la fiche, ou « jusqu'ou est-il descendu » est precisement ce qu'une image seule ne dit pas |
| 5 | **§4** | **Le vocabulaire de la tranche 10 tient, et il ne lui manque plus qu'UN muscle** | wger ne modelisait ni lombaires, ni avant-bras, ni adducteurs, tous atteints par des regles de nom (§9.25 n° 5). free-exercise-db nomme les trois, ce qui **retire cet instrument**. Il ne reste qu'`obliques`, que la source replie dans `abdominals`. La regle de nom survit pour lui seul et **ne se declenche QUE lorsque la source dit `abdominals`** : elle reattribue a l'interieur de la famille abdominale et ne peut pas voler un exercice de pectoraux a « Incline Dumbbell Flyes - With A Twist » — la raison pour laquelle le mot `twist` seul n'est pas dans le motif |
| 6 | **§4** | **Trois de leurs dix-sept muscles n'ont pas d'equivalent et sont PLACES, pas jetes** | `middle back` va sur `lats` — c'est ce qu'un pratiquant francais appelle du dos, et `body-map.ts` dessine deja `lats` sur la region haute qui couvre rhomboides et trapeze moyen ; l'envoyer sur `traps` allumerait la region des shrugs pour un rowing. `abductors` va sur `glutes`, `neck` sur `traps`. La regle est celle de la carte corporelle : **l'erreur contre laquelle on se protege est le FAUX NEGATIF** |
| 7 | **REGRESSION EVITEE** | **`tracksDuration` avait disparu du generateur reecrit** | La colonne que la tranche 11 a trouvee morte — lue en quatre endroits, ecrite nulle part — et dont le catalogue est le **seul** a poser une valeur non nulle sur une installation neuve. La perdre l'aurait remise a `0` partout, en silence, exactement comme `0009`. Rattrapee en relisant les tests avant de les faire passer, pas par un ecran en echec. 161 exercices chronometres, dont 8 dans le jeu par defaut |
| 8 | **D16, §7** | **`KEEP_IMAGES=1` rend le generateur re-executable sans redepenser 1746 requetes** | Le defaut reste un effacement complet, pour qu'un fichier perime ne puisse jamais survivre a un renommage — il resterait dans `assets/` reference par rien |
| 9 | **DEFAUT DANS LA TRADUCTION, trouve par un test** | **Quatre noms francais etaient en doublon, et un etait plausible et faux** | L'installation est **idempotente par nom** : deux entrees du meme nom feraient silencieusement ignorer la seconde. Et « Cable Incline Pushdown » avait ete traduit « Extension triceps » alors que sa source dit `primaryMuscles: ['lats']` et `force: 'pull'` — c'est un pull-over bras tendus. Trouve par un controle qui compare le muscle nomme dans le nom francais aux muscles reels de l'exercice, pas a la relecture |
| 10 | **§3** | **`scripts/exercise-names-fr.mjs`** | 876 noms ecrits un par un plutot qu'un moteur de regles. Un traducteur par jetons traite sept noms sur dix et produit des absurdites confiantes sur les trois autres : l'ordre des mots francais n'est pas l'ordre anglais, et la traine est idiomatique. **Un nom faux dans une bibliotheque n'est pas un ecran casse : c'est une etiquette plausible sur le mauvais mouvement**, le seul genre de faux que ce projet traite comme grave. Une cle manquante fait echouer le generateur au lieu de laisser passer de l'anglais |
| 11 | **D15** | **Le depot grossit de 30,8 Mo**, et c'est le prix assume | Chaque `checkout` de la CI les tire. C'est la contrepartie directe de « toute ligne a une image », et elle est ecrite ici plutot que decouverte au premier build lent |

### 9.28 Vignettes de liste, et un defaut du jeu d'essai (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§2.6** | **`media_uri` monte sur l'element de LISTE**, a cote des notes et de `tracks_duration` | Meme motif exactement, et c'est le seul qui autorise un element de liste a porter une colonne que personne ne liste : elle vient de la meme ligne et de la meme requete, donc la porter ne coute rien. Une lecture par rangee serait le cout par rangee que la tranche 4 a refuse en etendant l'ajout rapide a toute la bibliotheque |
| 2 | **DEFAUT, §3** | **Le jeu d'essai ecrivait douze exercices sans `media_uri`** | Un brouillon n'en porte pas — `media_uri` n'est deliberement pas un champ de `ExerciseDraft`, parce que rien a l'ecran ne l'edite. C'etait juste tant que le catalogue n'existait pas, et c'est devenu un defaut le jour ou il a existe. **Rien d'autre ne pouvait l'attraper** : les lignes etaient valides, la liste les montrait, les routines pointaient dessus ; la seule chose fausse etait une colonne qu'aucun test ne lisait. Meme classe que `tracks_duration` en `0009` et que `duration_seconds` en tranche 10 — **la troisieme fois** |
| 3 | **D15** | **Le jeu d'essai installe depuis le catalogue au lieu de taper des brouillons** | Il donne donc au developpement les memes lignes que l'application donne a un vrai utilisateur : memes muscles, meme materiel, memes photographies. L'ALIAS est conserve pour que les routines semees se lisent comme un plan d'entrainement et non comme des slugs, et le bloc porte le **vrai** nom de l'exercice — une ligne de routine lit son exercice en direct, donc une etiquette perimee la ferait contredire la ligne qu'elle designe |
| 4 | **D15** | **Cout assume : les increments par exercice du jeu d'essai disparaissent** | 1 kg pour les elevations laterales, 5 kg pour le squat. L'installation de catalogue copie un seul increment. Ils etaient illustratifs, et c'est la routine semee qui exerce vraiment la regle de progression |
| 5 | **TESTS** | **Trois tests neufs, verifies par mutation** | Que tout exercice seme porte sa photographie ; qu'aucun nom seme ne soit etranger au catalogue ; que semer deux fois ne double rien. La mutation — reintroduire un exercice cree a la main — fait rougir les trois |

### 9.29 Sous-onglets, liste de seances, et une regression de rendu (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§3, §9.22** | **`training-screen.tsx` se scinde en trois panneaux**, chacun portant SA requete | Un panneau non monte ne lit pas. C'est la propriete qui compte, pas le decoupage : ouvrir l'onglet ne declenche plus trois lectures dont deux que personne ne regarde. Precedent exact, Stats en tranche 8 |
| 2 | **DEFAUT, D16** | **552 rangees rendues d'un coup, chacune avec sa photographie** | La regression a ete introduite le matin meme en ajoutant la vignette, et elle n'etait invisible que parce que la bibliotheque etait petite avant le catalogue. Plafond de quarante, annonce. **La mesure qui manque est celle de l'appareil** : rien ici ne chronometre un rendu React Native, donc ce qui est corrige est la CAUSE — un nombre d'images decodees — et non un chiffre observe |
| 3 | **§2.6, §3** | **`listSessions` et `SessionListItem`** | Trois requetes groupees pour n'importe quel nombre de seances — tetes, segments par seance, comptes de series par seance — jamais un `readSession` par rangee. C'est la discipline de la tranche 4 sur l'ajout rapide et de la tranche 6 sur les tags de recette |
| 4 | **D9, D12** | **La duree d'une seance se somme sur ses SEGMENTS, jamais `endedAt - startedAt`** | Tout le propos de D12 : une seance laissee ouverte une nuit porte un long trou, et la difference entre ses deux bouts compterait la nuit. C'est la meme fonction que l'ecran de seance, donc la liste et la page ne peuvent pas se contredire sur la duree d'un entrainement |
| 5 | **TESTS** | **Deux implementations d'une meme question, tenues par un test et verifiees par mutation** | `readSession` parcourt les series d'une seance et compte `status === 'done'` ; `listSessions` compte la meme chose pour toutes en une requete groupee. Elles s'accordent sur tout exemple ecrit a la main et divergeraient le jour d'un quatrieme statut — la liste dirait « 3/8 » au-dessus d'une page disant « 4/8 », plausible des deux cotes. Muter la regle en `!== 'pending'` fait rougir les deux tests |
| 6 | **§5** | **`ActionSheetIOS` pour la bifurcation d'ajout** | Aucune dependance n'entre : c'est du coeur de React Native, un vrai `UIAlertController`. Meme controle que la tranche 5 pour choisir un modele de journee |
| 7 | **MESURE** | **L'installation par defaut coute 170 ms pour 552 exercices** (better-sqlite3) | Enregistree dans un test, avec une borne **large** et assumee comme telle : better-sqlite3 sur un portable n'est pas expo-sqlite sur un telephone, donc le nombre ne promet rien de l'appareil. Ce qu'il attrape est un changement d'ORDRE — une lecture remise dans la boucle ferait passer ca en secondes |
| 8 | **POINT OUVERT NOMME** | **Une bibliotheque installee AVANT la bascule de catalogue ne recoit pas la nouvelle** | Le drapeau `catalog_seeded_at` dit « la question a ete posee », pas « ce catalogue-ci est installe ». C'est voulu (§5.3 : ne jamais defaire une suppression deliberee), et la consequence est qu'un changement de source ne rattrape pas les installations existantes. Le bouton de reinitialisation est la reponse, et elle est ecrite plutot que laissee a deviner |

### 9.30 Fenetrage des listes (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **D16** | **Amendement etroit : `FlatList` entre, une bibliotheque de listes reste ecartee** | D16 disait « quelques centaines de lignes au plus, vues standard ». La premiere moitie n'a jamais ete le probleme ; c'est la seconde qui tombe. `FlatList` est du coeur de React Native : aucune dependance, aucun cycle CI, et le risque que D16 visait — un moteur de liste tiers sur le chemin critique — n'est pas pris |
| 2 | **§3** | **`core/ui/virtual-list.ts`**, une constante et ses raisons | Les quatre nombres ne sont pas interchangeables et il fallait l'ecrire une fois : `initialNumToRender` decide de la vitesse d'ouverture, `maxToRenderPerBatch` de la fluidite du defilement, et **`windowSize` est la moitie « decharge » de la demande** — c'est lui qui demonte les rangees eloignees et libere leurs photographies. Son defaut vaut 21 ecrans, soit presque toute une liste de huit cents lignes |
| 3 | **D16** | **`removeClippedSubviews` n'est PAS pose, et c'est un refus** | C'est un second mecanisme, plus brutal, qui detache des vues natives ; il est documente comme peu fiable sur iOS et son mode de panne est **une rangee blanche qui ne revient jamais**. `windowSize` demonte a travers React et revient en rendant. Un seul mecanisme, celui sur lequel on peut raisonner |
| 4 | **D16** | **Pas de `getItemLayout`, et c'est un choix** | Il eviterait de mesurer et permettrait de sauter a un index exact. Il exige aussi que chaque rangee fasse la hauteur annoncee — or ces rangees portent une photographie, un bouton etoile et deux lignes de texte dont **rien ici ne peut mesurer la hauteur reelle**. Un `getItemLayout` faux n'avertit pas : il place le defilement au mauvais endroit et derive d'autant plus que la liste est longue |
| 5 | **§4, §3** | **`core/ui/list-card.tsx` : la carte est reconstruite PAR ses rangees** | Toutes les listes du projet dessinent une carte autour d'un `map`. Cette forme cesse de marcher des qu'une liste est virtualisee : les rangees ne sont plus toutes la, et la carte devrait etre le conteneur de defilement — ou vivent le champ de recherche et les filtres. Chaque rangee peint donc la surface et les deux bords lateraux, la premiere arrondit et ferme le haut, la derniere le bas |
| 6 | **PIEGE** | **Le separateur va DANS la rangee, jamais en `ItemSeparatorComponent`** | Un `FlatList` dessine ses separateurs ENTRE les items, donc en freres des rangees — et un frere ne porte pas les bords lateraux qu'elles peignent. La carte aurait un trou dans ses deux bordures verticales a chaque separateur |
| 7 | **PIEGE** | **`ListHeaderComponent` recoit un ELEMENT, jamais une fonction en ligne** | Une fonction fleche ecrite dans le JSX est un **nouveau type de composant a chaque rendu**, donc React demonte et remonte le sous-arbre — lequel contient le champ de recherche, qui perdrait le focus a chaque frappe. Un element garde le type stable et se reconcilie en place. Meme famille que le `key` du carrousel de la tranche 3 : l'identite decide du remontage |
| 8 | **§4** | **`offerableCatalog` sort du composant** | « Quels exercices peuvent encore etre proposes » decide ce que l'ecran ajoute, ce que ses filtres annoncent et ce que son compteur dit : trois lectures d'une meme soustraction, libres de diverger si elles vivent dans le JSX. Le §4 garde les decisions metier hors des composants, et ca la rend testable |
| 9 | **CONSEQUENCE** | **Les deux plafonds de quarante disparaissent**, ainsi que la ligne « N autres correspondent » | Elle n'avait de sens qu'a cote d'un plafond. Ce qui la remplace n'est pas un autre message : c'est que la liste entiere est atteignable en defilant |

### 9.31 Deux ecritures neuves pour la table de seance (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§2.6, D12** | **`reopenSet` : `done` -> `pending`, et `completed_at` est EFFACE** | Ce n'est pas de la comptabilite. `completed_at` est l'instant depuis lequel le minuteur de repos compte, et la colonne sur laquelle `ix_set_exercise` est bati : une serie qui n'est plus faite ne doit ni ancrer un repos ni apparaitre dans un historique de ce qui a ete souleve. **Les valeurs enregistrees restent** — rouvrir pour corriger des repetitions en perdant la charge ferait couter la correction plus cher que l'erreur |
| 2 | **§2.6, D12** | **`setSetRir` : ecrit `actual_rir` SANS toucher `status`** | Un RIR sur une serie en attente est un plan, sur une serie faite une correction, et **ni l'un ni l'autre n'affirme que la serie a eu lieu**. Cette affirmation a son propre bouton. Un test le fixe dans les deux sens, parce que refondre les deux est la simplification tentante |
| 3 | **D12** | **Le RIR s'ecrit IMMEDIATEMENT, comme une validation et pas comme une frappe** | La ligne entre les deux rythmes de D12 n'est pas la table touchee : c'est si l'acte est **discret**. Taper une charge est un flux de frappes et se differe ; choisir un RIR parmi huit boutons est une decision, donc elle s'ecrit tout de suite et survit a un arret force pour la meme raison |
| 4 | **§4** | **L'etat de frappe est LACHE a la validation** | Sans ca, une serie validee sans avoir touche aux champs garde `{reps: null}` dans la carte locale et la cellule retombe sur son texte indicatif — **le meme nombre, dans la couleur pale**, donc une serie enregistree se lit comme une serie vide. Apres un acte discret la ligne stockee fait autorite ; la carte locale n'existe que le temps d'un debounce |
| 5 | **§3** | **`core/…/rir-picker.tsx` remplace `rir-row.tsx`, qui est SUPPRIME** | Il n'avait plus d'appelant, et du code sans appelant est un piege pour qui le rebranchera en le croyant utilise — la regle que `readFirstWeightDate` avait deja suivie en tranche 8 |
| 6 | **§4** | **`colRir` et `colCheck` vivent dans `setColumns`**, avec les colonnes partagees | La table des routines a quatre colonnes et s'arrete au RIR cible ; une ligne de seance en a cinq. Elles sont declarees la pour que l'en-tete que l'ecran dessine a la main ne puisse pas deriver des cellules en dessous — meme raison que `colSet` et `colValue` |
| 7 | **CABLAGE MORT, SIGNALE ET NON ELARGI** | **`onSkip` est passe a `BlockCard` et n'est jamais rendu** | Prexistant a ce lot : le statut `skipped` existe en base, `setSkipped` est ecrit et teste, et **aucun controle ne l'atteint**. La ligne le dessine donc en lecture seule. Non corrige ici parce qu'inventer un geste de saut serait elargir la demande ; nomme pour que ca ne passe pas pour du support |

### 9.32 La lecture du precedent (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§2.6, §3** | **`readPreviousSets` : deux requetes, quel que soit le nombre de series** | Une pour trouver la seance, une pour lire ses series. La discipline de la tranche 4 sur l'ajout rapide : une lecture par rangee est ce qui fait qu'un ecran qui s'ouvre en un dixieme de seconde s'ouvre en deux |
| 2 | **D9** | **L'appariement est `(exercice, set_index)`, JAMAIS la position** | `position` est l'ordre d'execution et il **bouge** : ajouter un exercice en cours de seance, retirer une serie, reordonner un superset le decalent tous. Une colonne qui apparierait discretement un developpe couche avec le curl de la semaine derniere parce qu'une ligne a ete inseree au-dessus est le genre de faux plausible et invisible que ce projet traite comme grave. `set_index` est le TOUR, donc la paire reste unique meme dans un superset |
| 3 | **D5/R4** | **`previousFor` : l'id d'abord, le nom fige en repli — et la premiere version se trompait** | Supprimer un exercice annule son id sur **toutes** les series qui l'ont jamais utilise et le retire des routines. Quelqu'un qui supprime un squat puis le recree obtient un id neuf : un appariement par id seul afficherait une colonne vide pour du travail reellement fait, ce qui se lit comme un defaut et non comme une consequence. Le nom fige fait le pont. **Une seule fonction porte la regle**, parce qu'une seconde ecriture se tromperait precisement sur le cas que personne n'essaie a la main |
| 4 | **DEFAUT DANS UN TEST, trouve en l'ecrivant** | **`anExercise()` cree un exercice NEUF a chaque appel** | Trois seances construites par trois appels portaient trois exercices differents, donc la colonne ne trouvait rien **en ayant l'air parfaitement correcte**. C'est exactement le defaut que le test existe pour attraper, rencontre d'abord dans le test lui-meme. Un `beforeEach` partage desormais un seul exercice, et le commentaire dit pourquoi |
| 5 | **§2.6** | **Seules les series `done` de la seance precedente sont lues** | Une ligne en attente porte ce qui avait ete tape avant qu'on l'abandonne. L'afficher comme « ce que tu as fait la derniere fois » mettrait un nombre que personne n'a realise dans la colonne que quelqu'un s'apprete a essayer de battre |
| 6 | **§2.6** | **`session.routine_id` est expose sur `SessionView`** | Il reste **informatif** comme la colonne le dit : le snapshot du nom est ce que l'ecran affiche. Il est expose parce que la question est « la derniere fois que CETTE routine a ete faite », qui porte sur la routine et non sur son nom — deux seances d'une routine renommee entre-temps sont la meme routine |
| 7 | **§4** | **`setSetType` : immediat, comme la validation et le RIR** | La ligne entre les deux rythmes de D12 n'est pas la table touchee, c'est si l'acte est **discret** |
| 8 | **REGRESSION INTRODUITE LA VEILLE** | **Epingler le bandeau lui a fait perdre son encart** | `contentInsetAdjustmentBehavior="automatic"` poussait le contenu sous l'en-tete transparent tant que le bandeau etait DANS le defilement. Dehors, plus rien ne le faisait. La page declare donc son encart et le defilement passe en `never` — la sortie que le §14.11 avait deja trouvee, appliquee a un cas qu'elle n'avait pas prevu |

### 9.33 La vibration remplace la notification de repos (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§5, D14** | **Plus aucune notification de repos ; `Vibration` du coeur de React Native** | Aucune dependance n'entre. Un son en aurait demande une, hors du §5, avec cycle CI et accord explicite |
| 2 | **D14, §2.2** | **`rest_alert_enabled` est une ligne `setting`, PAS une cinquieme `NOTIFICATION_KIND`** | Rien n'est programme ni annule, donc le planificateur quotidien n'a rien a posseder. C'est exactement l'ajout que le commentaire de `notification_setting` et le catalogue d'export invitaient tous deux, et que la tranche 11 a corrige : `diffSchedule` annule tout ce qu'il possede et qui n'est pas dans le plan |
| 3 | **§3** | **`restNotificationId` et `REST_NOTIFICATION` sont SUPPRIMES** | Sans appelant depuis ce changement, et du code sans appelant est un piege pour qui le rebranchera en le croyant utilise — la regle suivie depuis `readFirstWeightDate`. Un commentaire a leur place dit ce qu'ils portaient et ce qu'il faudrait refaire si une notification revenait |
| 4 | **TESTS** | **L'invariant qu'ils gardaient change de sujet, pas de valeur** | Deux tests assertaient qu'aucune `NOTIFICATION_KIND` n'est un prefixe de `rest:`. Il n'y a plus de `rest:`, donc le contre-exemple redevient un identifiant **etranger** — et cette fois deliberement : ce qui est teste est la regle du planificateur lui-meme, qui n'a pas besoin d'une seconde fonctionnalite pour etre vraie |
...
### 9.35 Tranche 12 (19/09/2026)

| No | Section | Amendement | Motif |
| --- | --- | --- | --- |
| 1 | **§3, D9** | **`data/history-reads.ts` : UNE lecture, TROIS plis** | Les cinq graphiques du §10.1, les quatre records du §10.1 et la condition du §10.4 sont trois questions sur les memes lignes — les series de travail qu'un exercice a jamais recues. Trois lectures seraient trois occasions de ne plus etre d'accord sur ce qui compte, qui est le defaut que ce projet poursuit depuis le pre-remplissage de la quantite |
| 2 | **D5/R4, §5.3** | **`readExerciseHistory` lit par IDENTIFIANT seul ; `previousFor` garde son repli sur le nom fige** | Deux regles delibrement differentes. Le §5.3 promet que supprimer un exercice rompt « definitivement » la continuite statistique et l'utilisateur lit cette phrase avant de confirmer ; faire remonter ses anciennes series par leur nom lui rendrait en silence les records qu'on lui a dit avoir detruits. Le repli reste sur UNE colonne d'UN ecran, ou il est une commodite d'affichage |
| 3 | **§2.6** | **La requete d'historique ne filtre PAS le `status`** | Les graphiques veulent les series validees et le disent par `countsTowardsVolume` ; le §10.4 a besoin des autres, puisque c'est une serie non faite qui rend sa condition fausse. Filtrer dans la requete ferait passer une seance abandonnee pour une seance parfaite |
| 4 | **§4** | **`SESSION_ORDER` : l'ordre des seances en une seule ecriture** | Il en existait deja deux — `listSessions` et `readPreviousSets` — et cette tranche en aurait fait une troisieme. Le cas ou deux ecritures divergent est la seance enregistree pour hier, que personne n'essaie a la main. Le precedent est le `ROW_NUMBER` de la tranche 4, tenu caractere pour caractere contre `readLastEntryForFood` |
| 5 | **§2.6, D12 — DEFAUT STRUCTUREL EVITE** | **`touchSession` ne prolonge pas une seance `done`** | Le §10.5 rend une seance terminee editable et le §5.3 n'y met aucune limite de temps. Les corrections passent par les memes fonctions, et trois semaines depassent largement les trente minutes de D12 : sans garde, corriger une charge apres coup ouvrait un segment NEUF et le « temps actif » grossissait du temps passe a corriger une faute de frappe — sur le chiffre meme que le §10.6 dessine. Le garde est dans `touchSession` et non a chaque site d'ecriture, pour la raison que son propre commentaire donnait deja des trente minutes |
| 6 | **§2.6, D3 — DECISION** | **`completed_at` d'une serie validee apres la fin prend l'`ended_at` de la seance** | L'horloge du jour placerait la serie une semaine apres la seance a laquelle elle appartient, or `ix_set_exercise` est bati sur cette colonne. Ce n'est pas la verite — personne ne sait quand la serie a ete faite — mais c'est a l'interieur de la seance, qui est la propriete dont tout depend en aval. Sans danger precisement parce que l'instant ne decide pas du jour : `session.date` le fait |
| 7 | **D9** | **Rien de derive n'est stocke, et c'est ce qui rend l'edition de l'historique bon marche** | D9 nomme deja « volume, 1RM d'Epley, records personnels, suggestion de double progression » parmi les recalcules. Corriger une charge d'un mois deplace les records a la lecture suivante, sans invalidation ecrite a la main. Un test le fixe plutot que de le proclamer |
| 8 | **D9, D13 — DECISION** | **Trois series se regroupent par MAXIMUM, quatre par MOYENNE** | D9 tranche les totaux et pas les maxima. Un graphique intitule « charge maximale » dont les points seraient des moyennes contredit son propre titre. Les figures par seance prennent la moyenne, qui survit en outre a un seau partiel en bout de plage la ou une somme se lirait comme un effondrement |
| 9 | **D9, D13** | **Le grain suit la regle de span de D9, et n'est PAS assoupli** | Un an d'un exercice fait deux fois par semaine, c'est une centaine de points — deja sous les deux cents de D13, donc rien ne forcait le regroupement hebdomadaire. Il est applique quand meme : D9 est normatif et n'exempte pas les series creuses, et une regle conditionnelle (« regrouper seulement s'il y a trop de points ») est une regle que quelqu'un doit se rappeler |
| 10 | **§3, D10** | **`components/session-block-card.tsx` : `BlockCard` sort a son deuxieme utilisateur reel** | Le §10.2 exige deja que les deux tables se ressemblent, et deux ecritures s'accorderaient sur un bloc ordinaire pour diverger sur un superset avec echauffement. Trois props disent ce qui differe entre la seance en direct et la seance terminee, et chaque absence signifie quelque chose plutot que d'economiser du travail |
| 11 | **§3, D10** | **`ExerciseChart` devient `SessionSeriesChart` a son deuxieme utilisateur reel** | Ce qui differait entre la page d'exercice et le tableau de bord etait entierement la METRIQUE, donc la metrique est sortie. Un graphique de volume de seance sur les deux ecrans ne doit pas etre deux images d'un meme nombre |
| 12 | **§3** | **`app/(tabs)/training/session/[id].tsx` a cote de `session.tsx`** | Il n'y a qu'une seance en cours (D12), donc c'est « la » seance et elle n'a pas besoin d'identifiant ; une seance terminee est une parmi beaucoup. Deux destinations, deux routes : le bandeau mene a la premiere et la liste Seances a la seconde. Verifie dans l'export lisible, pas suppose : les deux coexistent dans l'arbre de routes |
| 13 | **D13** | **Le graphique croise aligne ses deux series par `bucketOf`** | L'axe vient de la lecture du poids, dense, et le volume est indexe par la fonction meme par laquelle `readWeightSeries` regroupe. C'est exactement pourquoi `core/db/date-bucket` vit dans le noyau : deux series d'un graphique tombant sur des seaux distants de six jours mettrait ici une semaine d'entrainement en face du mauvais poids |
| 14 | **D13** | **Le graphique croise force le grain a la semaine au minimum** | Un volume par seance contre un poids qui bouge sur des semaines, ce sont deux nuages — l'argument que `WeightCrossChart` fait deja a propos des series brutes |
| 15 | **§3** | **Le poids du graphique croise passe par `readWeightSeries` et `smoothSeries`**, les fonctions du volet Poids | Pas une seconde ecriture de « poids lisse ». Les deux graphiques tracent donc la meme courbe pour une plage donnee plutot que deux courbes plausibles |
| 16 | **D15** | **Le generateur ecrit un historique de seances** | Le quatrieme travail que D15 lui donne — « verifier les performances sur de longs historiques » — et le seul jamais rempli ; `seedRoutines` le differait en toutes lettres jusqu'a cette tranche. Par les fonctions d'ecriture ordinaires, donc les SEGMENTS sont ecrits par `touchSession` plutot qu'inventes et `ux_session_active` est respecte gratuitement. L'instant vient de `toDayNumber`, jamais d'un `Date` (D3) |
| 17 | **§4** | **La prop morte `onSkip` est retiree ; `setSkipped` et `useSkipSet` restent, signales** | Declaree, typee, passee et ignoree par le composant : une prop qu'on croit branchee est un piege. L'ecriture elle-meme reste, sur le precedent de `sweepCache` — `status = 'skipped'` est un etat specifie que le reste du code lit, seul le chemin pour l'atteindre manque |
| 18 | **AUCUNE MIGRATION** | **La tranche n'en porte pas, et la regle le justifie** | Une migration porte ce qui ne peut pas etre ajoute plus tard. Rien a stocker : D9 nomme records, volume, 1RM, suggestion et duree parmi les recalcules. Rien a indexer : `ix_set_exercise (exercise_id, completed_at)` est exactement l'index de la page d'exercice, `ix_session_date` celui du volet. Et le corollaire joue dans le bon sens — **un index n'est pas irreversible**, donc rien ne justifiait d'en embarquer un par anticipation. Consequence : le catalogue d'export ne bouge pas et le compte de tables non verifiees sur l'appareil reste a **dix-huit** |
| 19 | **OBSERVATION SUR DU CODE LIVRE, non corrigee** | **`weightPanel` lisse sur sept POSITIONS quel que soit le grain** | Sur « 1 an », regroupe par semaine, c'est donc une moyenne mobile de sept SEMAINES. Le commentaire de `showRaw` suppose pourtant qu'a grain grossier serie brute et serie lissee « se confondent visuellement ». Les deux ne peuvent pas etre vrais. Le graphique croise de cette tranche emprunte le meme chemin volontairement : etre bizarre a l'identique vaut mieux qu'etre bizarre differemment. A trancher |
