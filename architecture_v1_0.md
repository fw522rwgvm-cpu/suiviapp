...
| 5 | **DIRECTION iOS 26** | **La barre d'accessoire porte le materiau, ce qui est dessus ne le porte pas** | Demande explicite (specs §14.26 n° 4). « Jamais de `GlassButton` dans un en-tete natif » tient parce qu'un en-tete pose **deja** le materiau d'UIKit derriere ce qu'on lui donne. Un `InputAccessoryView` est un conteneur **vide** : rien ne lui est applique, donc `GlassView` y est la seule facon d'obtenir le materiau. La ligne de partage n'est pas « natif contre maison », c'est **« la surface porte-t-elle deja un materiau »** — et une fois la bande en verre, des capsules dessus seraient du verre dans du verre, ce qui est la meme regle appliquee un cran plus bas |
| 6 | **RESERVE INSCRITE** | **Une barre d'accessoire ne peut pas etre « exactement » celle du systeme** | iOS n'en expose aucune a demander — ni par React Native, ni par UIKit hors d'une vue web, d'ou vient celle de Safari. Tout ce qui est dans cette barre est dessine ici. Dit plutot que laisse croire, comme la tranche 3 l'a fait pour le balayage de suppression et le retour par glissement. **`GlassButton` a gagne un `disabled` pour les capsules, puis l'a reperdu avec elles** : une API sans appelant est un piege pour qui la trouvera |
